...
module.exports = function (log) {
  ...
  var install = function (project, options) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      fs.writeFileSync(project.tempPath + '/package.json', npmJson);

      if (project.npm.shrinkwrap) {
        var shrinkwrap = JSON.stringify(project.npm.shrinkwrap);
        fs.writeFileSync(project.tempPath + '/npm-shrinkwrap.json', shrinkwrap);
      }
      ...
      npmCli.on('close', function (code) {
        ...
        fs.unlink(project.tempPath + '/package.json', function () {
          fs.unlink(project.tempPath + '/npm-shrinkwrap.json', function () {
            resolve();
          });
        });
